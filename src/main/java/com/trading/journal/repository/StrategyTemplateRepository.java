...
public interface StrategyTemplateRepository extends JpaRepository<StrategyTemplate, Long> {
     ...
     * 계좌별 템플릿 목록 (또는 전체 공용)
     */
    @Query("SELECT t FROM StrategyTemplate t WHERE " +
     ...
     */
    @Query("SELECT t FROM StrategyTemplate t ORDER BY t.isDefault DESC, t.usageCount DESC, t.name ASC")
     ...
     * 기본 템플릿 해제 (특정 계좌)
     */
    @Query("UPDATE StrategyTemplate t SET t.isDefault = false WHERE " +
           "(t.accountId IS NULL OR t.accountId = :accountId)")
