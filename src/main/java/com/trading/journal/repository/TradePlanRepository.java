...
public interface TradePlanRepository extends JpaRepository<TradePlan, Long> {
    ...
    List<TradePlan> findByAccountIdAndStatusOrderByCreatedAtDesc(Long accountId, TradePlanStatus status);
    ...
    // 대기 중인 플랜 (만료 임박 순)
    @Query("SELECT p FROM TradePlan p WHERE p.status = 'PLANNED' " +
    ...
    // 만료 플랜 일괄 업데이트
    @Modifying
    @Query("UPDATE TradePlan p SET p.status = 'EXPIRED', p.updatedAt = :now " +
    ...
    @Query("SELECT p.strategy, COUNT(p), " +
    ...
    // 실행된 플랜 중 결과가 있는 것들
    @Query("SELECT p FROM TradePlan p WHERE p.status = 'EXECUTED' " +
    ...
    @Query("SELECT AVG(p.actualRMultiple) FROM TradePlan p " +
           "WHERE p.status = 'EXECUTED' AND p.actualRMultiple IS NOT NULL")
    Double getAverageRMultiple();

    // 기간별 조회
    @Query("SELECT p FROM TradePlan p WHERE p.createdAt BETWEEN :start AND :end ORDER BY p.createdAt DESC")
    List<TradePlan> findByDateRange(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
