...
public interface DividendRepository extends JpaRepository<Dividend, Long> {
    ...
    @Query("SELECT d FROM Dividend d WHERE d.account.id = :accountId AND d.stock.symbol = :symbol ORDER BY d.paymentDate DESC")
    List<Dividend> findByAccountIdAndStockSymbol(@Param("accountId") Long accountId, @Param("symbol") String symbol);

    @Query("SELECT SUM(d.netAmount) FROM Dividend d WHERE d.account.id = :accountId AND d.paymentDate BETWEEN :startDate AND :endDate")
