...
public interface BenchmarkPriceRepository extends JpaRepository<BenchmarkPrice, Long> {
    ...
    Optional<BenchmarkPrice> findByBenchmarkAndPriceDate(
            BenchmarkType benchmark, LocalDate priceDate);
    ...
    /** 일간 수익률 조회 (기간별) */
    @Query(
            "SELECT b.dailyReturn FROM BenchmarkPrice b "
    ...
    @Query(
            "SELECT b FROM BenchmarkPrice b "
                    + "WHERE b.benchmark = :benchmark "
                    + "AND b.priceDate IN :dates "
                    + "ORDER BY b.priceDate ASC")
    List<BenchmarkPrice> findByBenchmarkAndPriceDates(
            @Param("benchmark") BenchmarkType benchmark, @Param("dates") List<LocalDate> dates);

    /** 기간 내 종가 조회 */
    @Query(
            "SELECT b.priceDate, b.closePrice FROM BenchmarkPrice b "
                    + "WHERE b.benchmark = :benchmark "
                    + "AND b.priceDate BETWEEN :startDate AND :endDate "
                    + "ORDER BY b.priceDate ASC")
