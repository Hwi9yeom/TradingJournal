...
public interface TradingJournalRepository extends JpaRepository<TradingJournal, Long> {
     ...
     * 계좌 + 날짜 (계좌 null 포함)
     */
    @Query("SELECT j FROM TradingJournal j WHERE " +
     ...
     * 평균 집중도 점수 조회
     */
    @Query("SELECT AVG(j.focusScore) FROM TradingJournal j WHERE " +
           "(:accountId IS NULL OR j.accountId = :accountId) " +
           "AND j.journalDate BETWEEN :startDate AND :endDate " +
     ...
     * 평균 규율 점수 조회
     */
    @Query("SELECT AVG(j.disciplineScore) FROM TradingJournal j WHERE " +
           "(:accountId IS NULL OR j.accountId = :accountId) " +
           "AND j.journalDate BETWEEN :startDate AND :endDate " +
     ...
     * 감정 상태별 통계 (오전)
     */
    @Query("SELECT j.morningEmotion, COUNT(j) FROM TradingJournal j WHERE " +
           "(:accountId IS NULL OR j.accountId = :accountId) " +
           "AND j.journalDate BETWEEN :startDate AND :endDate " +
     ...
     * 감정 상태별 통계 (오후)
     */
    @Query("SELECT j.eveningEmotion, COUNT(j) FROM TradingJournal j WHERE " +
           "(:accountId IS NULL OR j.accountId = :accountId) " +
           "AND j.journalDate BETWEEN :startDate AND :endDate " +
     ...
     * 일지 개수 조회
     */
    @Query("SELECT COUNT(j) FROM TradingJournal j WHERE " +
           "(:accountId IS NULL OR j.accountId = :accountId) " +
           "AND j.journalDate BETWEEN :startDate AND :endDate")
