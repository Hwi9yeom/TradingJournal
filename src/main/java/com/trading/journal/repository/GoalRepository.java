...
public interface GoalRepository extends JpaRepository<Goal, Long> {
     ...
     * 곧 마감되는 목표 조회 (N일 이내)
     */
    @Query("SELECT g FROM Goal g WHERE g.status = 'ACTIVE' AND g.deadline BETWEEN :today AND :futureDate ORDER BY g.deadline ASC")
    List<Goal> findUpcomingDeadlines(@Param("today") LocalDate today, @Param("futureDate") LocalDate futureDate);
