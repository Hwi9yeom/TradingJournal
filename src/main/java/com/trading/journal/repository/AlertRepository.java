...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    /** 특정 기간 내 알림 조회 */
    @Query(
            "SELECT a FROM Alert a WHERE a.createdAt BETWEEN :start AND :end ORDER BY a.createdAt DESC")
    ...
    /** 우선순위 높은 읽지 않은 알림 조회 */
    @Query(
            "SELECT a FROM Alert a WHERE a.status = 'UNREAD' AND a.priority IN ('HIGH', 'CRITICAL') ORDER BY a.priority DESC, a.createdAt DESC")
