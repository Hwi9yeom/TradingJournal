...
public interface DisclosureRepository extends JpaRepository<Disclosure, Long> {
    ...
    @Query(
            "SELECT d FROM Disclosure d WHERE d.stock.symbol = :symbol "
    ...
    @Query(
            "SELECT d FROM Disclosure d WHERE d.isRead = false "
                    + "AND d.stock.id IN (SELECT p.stock.id FROM Portfolio p) "
                    + "ORDER BY d.receivedDate DESC")
    ...
    @Query(
            "SELECT COUNT(d) FROM Disclosure d WHERE d.isRead = false "
                    + "AND d.stock.id IN (SELECT p.stock.id FROM Portfolio p)")
    Long countUnreadDisclosuresForPortfolio();

    @Query(
            "SELECT d FROM Disclosure d WHERE "
