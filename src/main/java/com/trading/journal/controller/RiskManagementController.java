...
public class RiskManagementController {
    ...
    public ResponseEntity<RMultipleAnalysis> getRMultipleAnalysis(
        ...
        return ResponseEntity.ok(
                riskDashboardService.analyzeRMultiples(accountId, startDate, endDate));
    }
    ...
    public ResponseEntity<PositionSizingResultDto> calculatePositionSize(
        ...
        log.info(
                "포지션 사이징 계산: entry={}, stopLoss={}, method={}",
    ...
    public ResponseEntity<Map<String, BigDecimal>> getKellyCriterion(
        ...
        BigDecimal kellyPct =
                positionSizingService.calculateKellyPercentage(accountId, startDate, endDate);
        ...
        if (kellyPct != null && kellyPct.compareTo(BigDecimal.ZERO) > 0) {
            result.put(
                    "halfKelly",
                    kellyPct.divide(BigDecimal.valueOf(2), 2, java.math.RoundingMode.HALF_UP));
            result.put(
                    "quarterKelly",
                    kellyPct.divide(BigDecimal.valueOf(4), 2, java.math.RoundingMode.HALF_UP));
        } else {
    ...
    public ResponseEntity<AccountRiskSettingsDto> updateRiskSettings(
            @PathVariable Long accountId, @Valid @RequestBody AccountRiskSettingsDto dto) {
    ...
    public ResponseEntity<AccountRiskSettingsDto> updateAccountCapital(
            @PathVariable Long accountId, @RequestParam BigDecimal capital) {
