...
public class AnalysisController {
    ...
    public ResponseEntity<CorrelationMatrixDto> getCorrelation(
        ...
        CorrelationMatrixDto correlation =
                analysisService.calculateCorrelationMatrix(startDate, endDate);
        return ResponseEntity.ok(correlation);
    }

    /** 롤링 상관관계 조회 시간에 따른 두 종목 간 상관관계 변화 추적 */
    ...
    public ResponseEntity<RollingCorrelationDto> getRollingCorrelation(
        ...
        log.info(
                "Getting rolling correlation for {} vs {} from {} to {} with {}d window",
                symbol1,
                symbol2,
                startDate,
                endDate,
        ...
        RollingCorrelationDto rolling =
                analysisService.calculateRollingCorrelation(
    ...
    public ResponseEntity<PairCorrelationDto> getPairCorrelation(
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
        log.info(
                "Getting pair correlation for {} vs {} from {} to {}",
                symbol1,
                symbol2,
                startDate,
                endDate);
        ...
        PairCorrelationDto pair =
                analysisService.calculatePairCorrelation(symbol1, symbol2, startDate, endDate);
    ...
    public ResponseEntity<SectorCorrelationDto> getSectorCorrelation(
        ...
        SectorCorrelationDto sector =
                analysisService.calculateSectorCorrelation(startDate, endDate);
        return ResponseEntity.ok(sector);
    }

    /** 종합 리스크 메트릭스 조회 VaR, Sortino, Calmar 등 고급 리스크 지표 반환 */
    ...
    public ResponseEntity<RiskMetricsDto> getRiskMetrics(
        ...
        log.info(
                "Getting risk metrics from {} to {} for account {}", startDate, endDate, accountId);
    ...
    /** 거래 패턴 분석 연승/연패, 요일별/월별 성과, 보유 기간 분석 */
    @GetMapping("/patterns")
    public ResponseEntity<TradingPatternDto> getTradingPatterns(
            ...
            @RequestParam(required = false) Long accountId) {
        log.info(
                "Getting trading patterns from {} to {} for account {}",
                startDate,
                endDate,
                accountId);
    ...
    public ResponseEntity<TradingPatternDto.StreakAnalysis> getStreakAnalysis(
            ...
            @RequestParam(required = false) Long accountId) {
        log.info(
                "Getting streak analysis from {} to {} for account {}",
                startDate,
                endDate,
                accountId);
    ...
    public ResponseEntity<BenchmarkComparisonDto> compareToBenchmark(
            ...
            @RequestParam(required = false) Long accountId) {
        log.info(
                "Comparing portfolio to {} from {} to {} for account {}",
                benchmark,
                startDate,
                endDate,
                accountId);
        ...
        BenchmarkComparisonDto comparison =
                benchmarkService.compareToBenchmark(accountId, benchmark, startDate, endDate);
    ...
    public ResponseEntity<List<BenchmarkComparisonDto.BenchmarkSummary>> getBenchmarkSummaries() {
        ...
        List<BenchmarkComparisonDto.BenchmarkSummary> summaries =
                benchmarkService.getBenchmarkSummaries();
    ...
    public ResponseEntity<List<Map<String, String>>> getBenchmarkTypes() {
        ...
        List<Map<String, String>> types =
                Arrays.stream(BenchmarkType.values())
                        .map(
                                type -> {
    ...
    public ResponseEntity<SectorAnalysisDto> analyzeSectors(
        ...
        SectorAnalysisDto analysis =
                sectorAnalysisService.analyzeSectors(accountId, startDate, endDate);
        return ResponseEntity.ok(analysis);
    }
    ...
    /**
     * 포트폴리오 트리맵 데이터 조회 (Finviz 스타일) 기간별 성과를 시각화하기 위한 데이터 반환
     *
