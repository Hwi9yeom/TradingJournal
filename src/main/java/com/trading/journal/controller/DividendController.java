...
public class DividendController {
    ...
    public ResponseEntity<DividendDto> updateDividend(
            @PathVariable Long id, @RequestBody DividendDto dividendDto) {
    ...
    @Operation(
            summary = "배당금 목록 조회",
            description = "조건에 따라 배당금 목록을 조회합니다. 조건이 없으면 최근 1년 데이터를 반환합니다.")
    @ApiResponse(responseCode = "200", description = "배당금 목록 조회 성공")
    @GetMapping
    public ResponseEntity<List<DividendDto>> getDividends(
            @Parameter(description = "종목 심볼 (옵션)", example = "AAPL") @RequestParam(required = false)
                    String symbol,
