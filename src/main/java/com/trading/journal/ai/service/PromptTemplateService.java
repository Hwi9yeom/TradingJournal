...
public class PromptTemplateService {
     ...
     * 성과 분석 프롬프트 생성
     */
    public String buildPerformanceAnalysisPrompt(PeriodAnalysisDto periodAnalysis,
        ...
        if (riskMetrics != null) {
            ...
            prompt.append(String.format("- 리스크 등급: %s\n",
                    riskMetrics.getRiskLevel() != null ? riskMetrics.getRiskLevel().getLabel() : "N/A"));
            if (riskMetrics.getVar95() != null) {
                prompt.append(String.format("- 95%% VaR (일간): %s%%\n", riskMetrics.getVar95().getDailyVaR()));
            }
            prompt.append("\n");
        }
        ...
        if (tradingPattern != null) {
            ...
            if (tradingPattern.getStreakAnalysis() != null) {
                ...
                prompt.append(String.format("- 현재 스트릭: %d (양수=연승, 음수=연패)\n", streak.getCurrentStreak()));
                prompt.append(String.format("- 최대 연승: %d, 최대 연패: %d\n",
            ...
            if (tradingPattern.getHoldingPeriodAnalysis() != null) {
                ...
                prompt.append(String.format("- 수익 거래 평균 보유: %s일, 손실 거래 평균 보유: %s일\n",
     ...
     * 거래 복기 자동 생성 프롬프트
     */
    public String buildTradeReviewPrompt(TransactionDto sellTransaction, TransactionDto buyTransaction) {
        StringBuilder prompt = new StringBuilder();
        ...
        // 노트
        if (buyTransaction != null && buyTransaction.getNotes() != null && !buyTransaction.getNotes().isEmpty()) {
     ...
     * 리스크 경고 분석 프롬프트
     */
    public String buildRiskWarningPrompt(RiskMetricsDto riskMetrics,
        ...
        if (riskMetrics != null) {
            prompt.append("### 리스크 지표\n");
            prompt.append(String.format("- 리스크 등급: %s\n",
                    riskMetrics.getRiskLevel() != null ? riskMetrics.getRiskLevel().getLabel() : "미분류"));
            prompt.append(String.format("- 변동성: %s%%\n", riskMetrics.getVolatility()));
            prompt.append(String.format("- 최대 낙폭: %s%%\n", riskMetrics.getMaxDrawdown()));
            prompt.append(String.format("- 샤프 비율: %s\n", riskMetrics.getSharpeRatio()));
            prompt.append(String.format("- 손익비: %s\n", riskMetrics.getProfitFactor()));
            if (riskMetrics.getVar95() != null) {
                prompt.append(String.format("- 95%% VaR: 일간 %s%%, 월간 %s%%\n",
                        riskMetrics.getVar95().getDailyVaR(),
        ...
        if (riskDashboard != null) {
            ...
            prompt.append(String.format("- 일일 손익: %s원 (%s%%)\n",
                    formatNumber(riskDashboard.getTodayPnl()), riskDashboard.getTodayPnlPercent()));
            prompt.append(String.format("- 주간 손익: %s원 (%s%%)\n",
                    formatNumber(riskDashboard.getWeekPnl()), riskDashboard.getWeekPnlPercent()));
            prompt.append(String.format("- 총 오픈 리스크: %s원 (%s%%)\n",
                    formatNumber(riskDashboard.getTotalOpenRisk()), riskDashboard.getOpenRiskPercent()));

            if (riskDashboard.getDailyLossStatus() != null) {
                var status = riskDashboard.getDailyLossStatus();
                prompt.append(String.format("- 일일 손실 한도: %s%% (사용률 %s%%, 상태: %s)\n",
                        status.getLimit(), status.getPercentUsed(), status.getStatusLabel()));
            }
            if (riskDashboard.getPositionCountStatus() != null) {
                var status = riskDashboard.getPositionCountStatus();
                prompt.append(String.format("- 포지션 수: %s / %s개 (상태: %s)\n",
                        status.getCurrent(), status.getLimit(), status.getStatusLabel()));
            }
            prompt.append("\n");

            // 집중도 경고
            if (riskDashboard.getConcentrationAlerts() != null && !riskDashboard.getConcentrationAlerts().isEmpty()) {
                prompt.append("### 집중도 경고\n");
                for (var alert : riskDashboard.getConcentrationAlerts()) {
                    prompt.append(String.format("- %s (%s): %.1f%% (한도 %.1f%%)\n",
                            alert.getStockName(), alert.getAlertType(),
                            alert.getConcentration(), alert.getLimit()));
                }
                prompt.append("\n");
            }
        }
    ...
    public String buildStrategyOptimizationPrompt(BacktestResultDto backtestResult) {
        ...
        prompt.append(String.format("- 테스트 기간: %s ~ %s\n",
