...
public class DividendService {
    ...
    public DividendDto createDividend(DividendDto dto) {
        Stock stock =
                stockRepository
                        ...
                        .orElseThrow(
                                () -> new RuntimeException("Stock not found: " + dto.getStockId()));
    ...
    public DividendDto updateDividend(Long id, DividendDto dto) {
                            ...
                            .orElseThrow(
                                    () ->
                                            new RuntimeException(
                                                    "Stock not found: " + dto.getStockId()));
        ...
        if (dto.getDividendPerShare() != null)
            dividend.setDividendPerShare(dto.getDividendPerShare());
    ...
    public DividendSummaryDto getDividendSummary() {
        ...
        BigDecimal yearlyDividends =
                dividendRepository.getTotalDividendsByPeriod(yearStart, yearEnd);
        if (yearlyDividends == null) yearlyDividends = BigDecimal.ZERO;

        // 월평균 배당금 (최근 12개월)
        LocalDate twelveMonthsAgo = now.minusMonths(12);
        BigDecimal last12MonthsDividends =
                dividendRepository.getTotalDividendsByPeriod(twelveMonthsAgo, now);
        BigDecimal monthlyAverage =
                last12MonthsDividends != null
                        ? last12MonthsDividends.divide(
                                new BigDecimal("12"), 2, RoundingMode.HALF_UP)
                        : BigDecimal.ZERO;

        // 배당 수익률 계산 (연간 배당금 / 현재 포트폴리오 가치)
        BigDecimal currentPortfolioValue =
                portfolioAnalysisService.getPortfolioSummary().getTotalCurrentValue();
        BigDecimal dividendYield =
                currentPortfolioValue.compareTo(BigDecimal.ZERO) > 0
                        ? yearlyDividends
                                .divide(currentPortfolioValue, 4, RoundingMode.HALF_UP)
                                .multiply(new BigDecimal("100"))
                        : BigDecimal.ZERO;

        // TOP 배당 종목
        List<DividendSummaryDto.StockDividendDto> topStocks =
                getTopDividendStocks(yearStart, yearEnd);
    ...
    private List<DividendSummaryDto.StockDividendDto> getTopDividendStocks(
        ...
        for (Object[] result : results) {
            ...
            // 해당 종목의 배당 횟수
            List<Dividend> stockDividends =
                    dividendRepository.findByStockOrderByPaymentDateDesc(stock);
            int paymentCount = stockDividends.size();

            topStocks.add(
                    DividendSummaryDto.StockDividendDto.builder()
    ...
    private List<DividendSummaryDto.MonthlyDividendDto> getMonthlyDividends() {
        ...
        for (Object[] result : results) {
            ...
            monthlyDividends.add(
                    DividendSummaryDto.MonthlyDividendDto.builder()
                            .year(year)
                            .month(month)
                            .amount(amount)
                            .build());
    ...
    private DividendDto convertToDto(Dividend dividend) {
        BigDecimal taxRate =
                dividend.getTotalAmount().compareTo(BigDecimal.ZERO) > 0
                        ? dividend.getTaxAmount()
                                .divide(dividend.getTotalAmount(), 4, RoundingMode.HALF_UP)
                                .multiply(new BigDecimal("100"))
                        : BigDecimal.ZERO;
