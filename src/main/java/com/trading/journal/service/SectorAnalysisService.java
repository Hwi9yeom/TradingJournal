...
public class SectorAnalysisService {
    ...
    public SectorAnalysisDto analyzeSectors(
        ...
        List<SectorPerformance> performances =
                calculateSectorPerformance(transactions, positionMap);
        ...
        int classifiedStocks =
                (int) positionMap.keySet().stream().filter(s -> s.getSector() != null).count();
    ...
    public Stock updateStockSector(Long stockId, Sector sector, String industry) {
        Stock stock =
                stockRepository
                        .findById(stockId)
                        .orElseThrow(
                                () -> new IllegalArgumentException("Stock not found: " + stockId));
    ...
    private Map<Stock, PositionInfo> calculatePositions(List<Transaction> transactions) {
        ...
        for (Map.Entry<Stock, PositionInfo> entry : positionMap.entrySet()) {
            ...
            if (position.quantity.compareTo(BigDecimal.ZERO) > 0 && position.lastPrice != null) {
                ...
                BigDecimal avgCost =
                        position.totalCost.divide(position.quantity, 4, RoundingMode.HALF_UP);
                position.unrealizedPnl =
                        position.quantity.multiply(position.lastPrice.subtract(avgCost));
            }
        }
    ...
    private List<SectorAllocation> calculateSectorAllocations(
            Map<Stock, PositionInfo> positionMap) {
        ...
        for (Map.Entry<Stock, PositionInfo> entry : positionMap.entrySet()) {
            ...
            if (position.currentValue == null
                    || position.currentValue.compareTo(BigDecimal.ZERO) <= 0) {
                continue;
            }

            Sector sector = stock.getSector() != null ? stock.getSector() : Sector.OTHER;

            StockInSector stockInfo =
                    StockInSector.builder()
                            .symbol(stock.getSymbol())
                            .name(stock.getName())
                            .currentValue(position.currentValue)
                            .profitLoss(position.unrealizedPnl)
                            .profitLossPercent(
                                    position.totalCost.compareTo(BigDecimal.ZERO) > 0
                                            ? position.unrealizedPnl
                                                    .divide(
                                                            position.totalCost,
                                                            4,
        ...
        BigDecimal totalValue =
                sectorValues.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private List<SectorPerformance> calculateSectorPerformance(
        ...
        for (Transaction tx : sellTransactions) {
            Sector sector =
                    tx.getStock().getSector() != null ? tx.getStock().getSector() : Sector.OTHER;
        ...
        for (Map.Entry<Stock, PositionInfo> entry : positionMap.entrySet()) {
            Sector sector =
                    entry.getKey().getSector() != null ? entry.getKey().getSector() : Sector.OTHER;
            SectorStats stats = sectorStats.computeIfAbsent(sector, k -> new SectorStats());
        ...
        for (Map.Entry<Sector, SectorStats> entry : sectorStats.entrySet()) {
            ...
            BigDecimal winRate =
                    stats.tradeCount > 0
                            ? BigDecimal.valueOf(stats.winCount)
                                    .divide(
                                            BigDecimal.valueOf(stats.tradeCount),
                                            4,
                                            RoundingMode.HALF_UP)
                                    .multiply(BigDecimal.valueOf(100))
                            : BigDecimal.ZERO;
            BigDecimal contribution =
                    totalRealizedPnl.compareTo(BigDecimal.ZERO) != 0
                            ? stats.realizedPnl
                                    .divide(totalRealizedPnl.abs(), 4, RoundingMode.HALF_UP)
                                    .multiply(BigDecimal.valueOf(100))
                            : BigDecimal.ZERO;

            performances.add(
                    SectorPerformance.builder()
                            .sector(sector)
                            .sectorLabel(sector.getLabel())
                            ...
                            .bestTrade(
                                    stats.bestTrade != null
                                            ? stats.bestTrade.setScale(0, RoundingMode.HALF_UP)
                                            : null)
                            .worstTrade(
                                    stats.worstTrade != null
                                            ? stats.worstTrade.setScale(0, RoundingMode.HALF_UP)
                                            : null)
                            .build());
        }
    ...
    private List<SectorRotation> calculateSectorRotation(List<Transaction> transactions) {
        ...
        for (Transaction tx : transactions) {
            ...
            Map<Sector, BigDecimal> sectorValues =
                    monthlyPositions.computeIfAbsent(ym, k -> new HashMap<>());

            // 월말 기준 섹터별 가치 계산
            for (Map.Entry<Stock, BigDecimal> entry : runningPositions.entrySet()) {
                if (entry.getValue().compareTo(BigDecimal.ZERO) > 0) {
                    Sector sector =
                            entry.getKey().getSector() != null
                                    ? entry.getKey().getSector()
                                    : Sector.OTHER;
    ...
    private BigDecimal calculateHHI(List<SectorAllocation> allocations) {
        ...
        for (SectorAllocation alloc : allocations) {
            BigDecimal weight =
                    alloc.getWeight().divide(BigDecimal.valueOf(100), 4, RoundingMode.HALF_UP);
