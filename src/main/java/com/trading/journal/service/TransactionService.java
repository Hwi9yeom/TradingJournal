...
public class TransactionService {
    ...
    public TransactionDto createTransaction(TransactionDto dto) {
        ...
        Transaction transaction =
                Transaction.builder()
                        ...
                        .remainingQuantity(
                                dto.getType() == TransactionType.BUY ? dto.getQuantity() : null)
                        // 리스크 관리 필드
        ...
        if (dto.getType() == TransactionType.BUY && dto.getStopLossPrice() != null) {
            calculateAndSetRiskFields(
                    transaction,
                    dto.getPrice(),
                    dto.getStopLossPrice(),
                    dto.getTakeProfitPrice(),
                    dto.getQuantity());
        }
    ...
    private Stock createNewStock(String symbol) {
        try {
            ...
            Stock.StockBuilder stockBuilder = Stock.builder().symbol(symbol.toUpperCase());
            ...
            Stock stock =
                    Stock.builder().symbol(symbol.toUpperCase()).name(symbol.toUpperCase()).build();
            return stockRepository.save(stock);
        }
    }

    private TransactionDto convertToDto(Transaction transaction) {
        TransactionDto.TransactionDtoBuilder builder =
        ...
        if (transaction.getPrice() != null
            ...
            if (transaction.getStopLossPrice() != null) {
                BigDecimal stopLossPercent =
                        transaction
                                .getPrice()
                                .subtract(transaction.getStopLossPrice())
    ...
    /** 리스크 관련 필드 계산 및 설정 */
    private void calculateAndSetRiskFields(
            Transaction transaction,
    ...
    /** SELL 거래에 대한 R-multiple 계산 R-multiple = 실현손익 / 초기리스크 */
    ...
    /** 초기 리스크 계산 */
    public BigDecimal calculateInitialRisk(
            BigDecimal entryPrice, BigDecimal stopLossPrice, BigDecimal quantity) {
    ...
    public BigDecimal calculateRMultiple(BigDecimal realizedPnl, BigDecimal initialRisk) {
        if (realizedPnl == null
                || initialRisk == null
                || initialRisk.compareTo(BigDecimal.ZERO) == 0) {
            return null;
        }
