...
public class ChartGenerationService {
     ...
     */
    public byte[] generateEquityCurveChart(List<String> labels, List<BigDecimal> values) throws IOException {
    ...
    public byte[] generatePortfolioPieChart(Map<String, BigDecimal> holdings) throws IOException {
        ...
        holdings.forEach((symbol, value) ->
                dataset.setValue(symbol, value.doubleValue()));
     ...
     * 섹터별 배분 파이 차트 생성
     */
    public byte[] generateSectorAllocationChart(Map<String, BigDecimal> sectorWeights) throws IOException {
        DefaultPieDataset<String> dataset = new DefaultPieDataset<>();

        sectorWeights.forEach((sector, weight) ->
                dataset.setValue(sector, weight.doubleValue()));
     ...
     * 월별 손익 바 차트 생성
     */
    public byte[] generateMonthlyProfitChart(Map<String, BigDecimal> monthlyProfits) throws IOException {
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();

        monthlyProfits.forEach((month, profit) ->
                dataset.addValue(profit.doubleValue(), "손익", month));
     ...
     */
    public byte[] generateDrawdownChart(List<String> labels, List<BigDecimal> drawdowns) throws IOException {
        XYSeriesCollection dataset = new XYSeriesCollection();
     ...
     * 요일별 성과 바 차트 생성
     */
    public byte[] generateDayOfWeekChart(Map<String, BigDecimal> dayOfWeekReturns) throws IOException {
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
