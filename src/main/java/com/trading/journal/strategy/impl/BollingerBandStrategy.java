...
/** 볼린저 밴드 전략 - 하단 밴드 터치 후 반등: 매수 - 상단 밴드 터치 후 하락: 매도 */
@Data
@Builder
public class BollingerBandStrategy implements TradingStrategy {
    ...
    /** 표준편차 배수 */
    @Builder.Default private double stdDevMultiplier = 2.0;
    ...
    public Signal generateSignal(List<PriceData> prices, int index) {
        ...
        boolean lowerBandBounce =
                prevClose.compareTo(prevBands.lowerBand) <= 0
                        && currentClose.compareTo(currentBands.lowerBand) > 0;

        // 상단 밴드 터치 후 하락 (이전 종가가 상단 밴드 이상, 현재 종가가 상단 밴드 이하)
        boolean upperBandReject =
                prevClose.compareTo(prevBands.upperBand) >= 0
    ...
    private BollingerBands calculateBands(List<PriceData> prices, int index) {
        ...
        BigDecimal variance =
                sumSquaredDiff.divide(BigDecimal.valueOf(period), 6, RoundingMode.HALF_UP);
        BigDecimal stdDev = BigDecimal.valueOf(Math.sqrt(variance.doubleValue()));

        // 밴드 계산
        BigDecimal bandWidth = stdDev.multiply(BigDecimal.valueOf(stdDevMultiplier));

        return new BollingerBands(sma, sma.add(bandWidth), sma.subtract(bandWidth), stdDev);
    }
    ...
    public String getDescription() {
        return String.format(
                "%d일 볼린저 밴드 (%.1f σ) 기준, 하단 밴드 반등 시 매수, 상단 밴드 하락 시 매도", period, stdDevMultiplier);
    }
