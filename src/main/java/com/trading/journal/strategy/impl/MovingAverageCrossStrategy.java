...
/** 이동평균 교차 전략 - 골든크로스 (단기 MA > 장기 MA): 매수 - 데드크로스 (단기 MA < 장기 MA): 매도 */
@Data
@Builder
public class MovingAverageCrossStrategy implements TradingStrategy {

    /** 단기 이동평균 기간 */
    @Builder.Default private int shortPeriod = 20;

    /** 장기 이동평균 기간 */
    @Builder.Default private int longPeriod = 60;

    /** 이동평균 유형 (SMA, EMA) */
    @Builder.Default private MAType maType = MAType.SMA;
    ...
    public Signal generateSignal(List<PriceData> prices, int index) {
        ...
        boolean goldenCross =
                prevShortMA.compareTo(prevLongMA) <= 0 && shortMA.compareTo(longMA) > 0;

        // 데드크로스: 이전에 단기 > 장기였다가 현재 단기 < 장기
        boolean deadCross = prevShortMA.compareTo(prevLongMA) >= 0 && shortMA.compareTo(longMA) < 0;
    ...
    public String getDescription() {
        return String.format("%d일 이동평균이 %d일 이동평균을 상향 돌파하면 매수, 하향 돌파하면 매도", shortPeriod, longPeriod);
    }
