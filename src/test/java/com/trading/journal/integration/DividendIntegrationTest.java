...
class DividendIntegrationTest {
    ...
    void dividendFullFlowTest() throws Exception {
        // 1. 배당금 생성
        DividendDto createRequest = DividendDto.builder()
        ...
        String createResponse = mockMvc.perform(post("/api/dividends")
        ...
        // 5. 배당금 수정
        DividendDto updateRequest = DividendDto.builder()
                .dividendPerShare(new BigDecimal("0.30"))
                .quantity(new BigDecimal("120"))
                .memo("수정된 통합테스트 배당")
                .build();

        mockMvc.perform(put("/api/dividends/" + dividendId)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void createDividend_ValidationFailure() throws Exception {
        // Given - 잘못된 데이터
        DividendDto invalidRequest = DividendDto.builder()
                .stockId(null) // 필수값 누락
