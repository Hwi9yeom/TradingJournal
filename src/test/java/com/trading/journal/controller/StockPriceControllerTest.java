...
class StockPriceControllerTest {
    ...
    void getStockPrice_StockNotFound() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/stocks/price/INVALID")).andExpect(status().isNotFound());
    ...
    void getStockPrice_QuoteNotFound() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/stocks/price/AAPL")).andExpect(status().isNotFound());
    ...
    void getStockPrice_ServiceException() throws Exception {
        // Given
        when(stockPriceService.getStockInfo("AAPL"))
                .thenThrow(new RuntimeException("Service error"));

        // When & Then
        mockMvc.perform(get("/api/stocks/price/AAPL")).andExpect(status().isBadRequest());
