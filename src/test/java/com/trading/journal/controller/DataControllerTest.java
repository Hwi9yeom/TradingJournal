...
class DataControllerTest {
    ...
    @MockitoBean private DataImportService dataImportService;

    @MockitoBean private DataExportService dataExportService;

    @MockitoBean private FifoCalculationService fifoCalculationService;
    ...
    void importCsv_Success() throws Exception {
        ...
        MockMultipartFile file =
                new MockMultipartFile(
                        "file", "transactions.csv", "text/csv", "test data".getBytes());

        when(dataImportService.importFromCsv(any())).thenReturn(mockImportResult);

        // When & Then
        mockMvc.perform(multipart("/api/data/import/csv").file(file))
    ...
    void importCsv_EmptyFile() throws Exception {
        ...
        when(dataImportService.importFromCsv(any()))
                .thenThrow(new IllegalArgumentException("파일이 비어있습니다"));

        // When & Then
        mockMvc.perform(multipart("/api/data/import/csv").file(emptyFile))
    ...
    void importExcel_Success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(multipart("/api/data/import/excel").file(file))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.totalRows").value(10))
                .andExpect(jsonPath("$.successCount").value(8))
                .andExpect(jsonPath("$.failureCount").value(2));
