...
- [x] 다른 유저로 로그인 시 `GET /api/transactions`(전 계열)에서 내 거래만 반환
- [x] 타 유저 accountId로 `/api/transactions/account/{id}` 호출 시 403/401
- [x] 목표 페이지 CRUD/요약이 예외 없이 동작
- [x] 토큰 없이 `/ws/alerts` 연결 시 핸드셰이크 거부
- [x] 가격 알림이 알림 소유자 세션에만 전송
- [x] `./gradlew check` 통과 (Spotless, JaCoCo 포함)
